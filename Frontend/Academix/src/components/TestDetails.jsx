...
export const TestDetails = () => {
  ...
  const questionDetails = async () => {
    ...
    try {
      ...
      let questions = [];
      if (response.data?.question_type?.replace(/['"\\]/g, '') === 'tof') {
        // For ToF, we convert the object to an array
        questions = Object.entries(response.data.question_detail).map(([key, value]) => ({
          [key]: value,
        }));
      } else {
        questions = response.data?.question_detail ? response.data.question_detail : [];
      }

      setQuestionsData(questions);
      setQuestionName(response.data.question_name);
      setQuestionType(response.data.question_type.replace(/['"\\]/g, ''));
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    setScore(score);
    try {
      const response = await axios.post('http://16.171.33.87:8000/api/update_answers/', {file_id: fileId, answers: selectedAnswers, score: score},
        { withCredentials: true,
          headers: {
            Authorization: 'Token ' + localStorage.getItem('token'),
          },
  ...
  return (
    <div className="font-Inter text-[#313131] mx-auto w-11/12 lg:w-11/12">
              ...
              <div key={key} className="my-2">
                <label className="flex items-center space-x-3">
                  <input
                    ...
                    value={value} // Using the value for ToF answers
                    checked={selectedAnswers[questionKey] === value}
                    onChange={() => handleAnswerSelect(questionKey, value)}
              ...
              <div key={key} className="my-2">
                <label className="flex items-center space-x-3">
                  <input
                    ...
                    onChange={() => handleAnswerSelect(questionKey, key)}
                    className="form-radio h-5 w-5 text-blue-600"
                  />
      ...
      <div className="flex justify-between items-center mt-6">
        <button onClick={handlePrevious} disabled={currentQuestionIndex === 0} className="bg-gray-200 hover:bg-gray-300 text-gray-700 py-2 px-4 rounded-lg disabled:opacity-50">
          Previous
        </button>
        {currentQuestionIndex < questionsData.length - 1 ? (
          <button onClick={handleNext} className="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg">
            Next
          </button>
        ) : (
          <button onClick={handleSubmit}className="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-lg">
            Submit
          </button>
        )}
      </div>
